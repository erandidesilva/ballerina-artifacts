...
function insertNestedParentSqlError(json payload)(string responsePayload){
    ...
    try{
        transaction {
            ...
            while(i<length_1){
                ...
                system:println("6");
                sql:ClientConnector.update(oracleConnectionInstance, "insert into persons (tid, lastname, firstname, age) values (?, ?, ?, ?)", paramsPerson);
                system:println("7");
                i = i+1;
            }
            transaction {
            ...
            transaction {
                ...
                while(x<length_3){
                    ...
                    system:println("x5");
                    sql:ClientConnector.update(mssqlConnectionInstance, "Insert into personsinlanka(personid,firstname,lastname) values (?,?,?)", paramsPerson);
                    system:println("x6");
                    x = x+1;
                }
